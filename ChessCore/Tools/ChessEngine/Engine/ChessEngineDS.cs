...
{
    public class ChessEngineDS : IChessEngine
    {
        ...
        public NodeCE GetBestModeCE(string colore, BoardCE boardChess, int depthLevel = 8, int maxReflectionTimeInMinute = 2)
        {
            ...
            //}
            bestOfBest = FindBestMode(boardChess, depthLevel, cpuColor);
            return bestOfBest;
        }
        ...
        public NodeCE FindBestMode(BoardCE board, int depthLevel, string cpuColor)
        {
            ...
            foreach (var move in possibleMoves)
            {
                tasks.Add(Task.Run(() =>
                {
                    if (isReflextionLimitExpered()) return;
